...
Camera::Camera(const std::string & config_path)
{
  ...
  if (camera_name_ == "mindvision") {
    ...
    auto wb_r = tools::read<int>(yaml, "wb_r");
    auto wb_g = tools::read<int>(yaml, "wb_g");
    auto wb_b = tools::read<int>(yaml, "wb_b");
    camera_ = std::make_unique<MindVision>(exposure_ms, gamma, vid_pid, wb_r, wb_g, wb_b);
  }
