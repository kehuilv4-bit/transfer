...
namespace io
{
MindVision::MindVision(double exposure_ms, double gamma, const std::string & vid_pid, int wb_r, int wb_g, int wb_b)
...
void MindVision::open()
{
  ...
  CameraSetWbMode(handle_, FALSE);                         // 手动白平衡
  CameraSetGain(handle_, wb_r_, wb_g_, wb_b_);             // 设置白平衡RGB增益
  CameraSetMirror(handle_, 0, TRUE);                       // 水平翻转
  CameraSetMirror(handle_, 1, TRUE);                       // 垂直翻转（合计旋转180度）
